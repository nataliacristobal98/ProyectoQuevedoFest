## 3-Modelo Lógico
### 3.2-Normalización y desnormalización

- **Primera Forma Normal (1FN)**

    - Tabla de **ORGANIZACIÓNN**: Ya estaría en 1FN porque no hay atributos multivalor.
    
    - Tablas de **ARTISTAS**:
        
        - **SPEAKERS**: Podemos suponer que los speakers pueden dar más de una charla, por lo que sí habria
        un atributo multivalor.
        
        SPEAKER (ID_artista(PK), ID_speaker(PK), nombre, agencia, fecha_turno, ID_organizacion(FK), ID_agenda(FK))
        
        CHARLA (ID_speaker(PK)(FK), tema_charla(PK))
        
        - **PRESENTADOR**: Ya estaría en 1FN.
        
        - **SOLISTA** y **GRUPO**: Podemos suponer que el color_oficial sea solo uno para que no haya multivalor en él, 
        por lo que estaría en 1FN.
        
        - **ESTILISTA_CANTANTE**, **VESTUARIO** y **MAQUILLAJE**: Es multivalor, ya que un estilista puede estar a cargo de más de un cantante.
        Para solucionarlo podríamos llevar cada ID de vestuario y maquillaje a la tabla contraria.
        
        SOLISTA (ID_artista(PK), ID_cantante(PK), ID_solista(PK), nombre, agencia, fecha_turno, género_musical, color_oficial, fecha_debut, nombre_fanclub, ID_organizacion(FK), ID_agenda(FK), ID_maquillaje(FK), ID_vestuario(FK))
        
        GRUPO (ID_artista(PK), ID_cantante(PK), ID_grupo (PK), nombre, agencia, fecha_turno, género_musical, color_oficial, fecha_debut, nombre_fanclub, num_integrantes, ID_organizacion(FK), ID_agenda(FK), ID_maquillaje(FK), ID_vestuario(FK))

        - **VESTUARIO** y **MAQUILLAJE**: Tendríamos que dividirlo en dos tablas para que no hubiera multivalor
        
        MAQUILLADOR (ID_maquillador, nombre, contacto, red_social, material, especialidad)
        
        VESTUARIO (ID_vestuario, nombre, contacto, red_social, marca_ropa)

        - **FANCLUB**: No hay atributos multivalor
        
        - **FAN**: Como un solo fan pertenece a un solo fanclub no habría multivalor.
        
        - **PROYECTO**: Tampoco habría multivalor al ser organizado un proyecto por un solo fan.
        
    - Tablas de **INVITACIONES**:
        
        - **INVITACION**: Ya está en 1FN al ser una entrada exclusiva del día.
        
        - **ACTOR/ACTRIZ** e **INFLUENCER**: Suponiendo que el campo de trabajo puede adoptar varios valores (por ejemplo: un influencer
        puede trabajar en publicidad y modelaje a la vez) se formaría una tabla nueva.
        
        ACTOR/ACTRIZ ((ID_celebridad(PK), ID_actor_actriz, nombre, agencia)
        
        INFLUENCER ((ID_celebridad(PK), ID_influencer, nombre, red_social)
        
        CAMPO_TRABAJO (ID_celebridad(PK)(FK), campo(PK))
        
    - Tablas de **CARTELES**:
    
        - **DISEÑADOR**: Ya está en 1FN.
        
        - **CARTEL**: También está en 1FN ya que solo hay un diseñador encargado de un cartel.

    - Tablas de **AGENDA**:
    
        - **AGENDA**: No tiene atributos multivalor.
        
        - **HORARIO**: No habría tampoco multivalor.
        
        - **HORARIO_ARTISTA**: Al igual que con el cartel se pasaría la ID del horario a los artistas.
        
        SPEAKER (ID_artista(PK), ID_speaker(PK), nombre, agencia, fecha_turno, ID_organizacion(FK), ID_cartel(FK), ID_horario(FK))
                        
        SOLISTA (ID_artista(PK), ID_cantante(PK), ID_solista(PK), nombre, agencia, fecha_turno, género_musical, color_oficial, fecha_debut, nombre_fanclub, num_bailarines, ID_organizacion(FK), ID_maquillaje(FK), ID_vestuario(FK), ID_cartel(FK), ID_horario(FK))
                        
        GRUPO (ID_artista(PK), ID_cantante(PK), ID_grupo (PK), nombre, agencia, fecha_turno, género_musical, color_oficial, fecha_debut, nombre_fanclub, num_integrantes, ID_organizacion(FK), ID_maquillaje(FK), ID_vestuario(FK), ID_cartel(FK), ID_horario(FK))
        
        PRESENTADOR (ID_artista(PK), ID_presentador(PK), nombre, agencia, fecha_turno, ID_organizacion(FK), ID_cartel(FK), ID_horario(FK))

- **Segunda Forma Normal (2FN)**

    - Tabla de **ORGANIZACIÓN**: Estaría en 2FN porque no hay relación con clave compuesta.
    
    - Tabla de **ARTSITAS**:
        
        - **SPEAKER**, **PRESENTADOR**, **SOLISTA** y **GRUPO**: Las dependencias de estos campos serían las mismas.
        
        ID_artista -> ID_organización, ID_cartel, ID_horario
        
        De todas estas tablas podría derivar una tabla única propia de artistas, al tener que poner más de una columna de ID de identificacion según sea speaker, presentador o cantante
        podría ponerse que el valor puede ser NULL en estas columnas.
        
        ARTISTA (ID_artista(PK), ID_organización(FK), ID_cartel(FK), ID_agenda(FK), ID_speaker(PK)(FK), ID_presentador(PK)(FK), ID_cantante(PK)(FK))
        
        SPEAKER (ID_speaker(PK), nombre, agencia, fecha_turno)
                                
        SOLISTA (ID_cantante(PK), ID_solista(PK), nombre, agencia, fecha_turno, género_musical, color_oficial, fecha_debut, nombre_fanclub, num_bailarines, ID_maquillaje(FK), ID_vestuario(FK))
                                
        GRUPO (ID_cantante(PK), ID_grupo (PK), nombre, agencia, fecha_turno, género_musical, color_oficial, fecha_debut, nombre_fanclub, num_integrantes, ID_maquillaje(FK), ID_vestuario(FK))
                
        PRESENTADOR (ID_presentador(PK), nombre, agencia, fecha_turno)

        - **SOLISTA** y **GRUPO**: Dependerían de su ID_cantante.
        
        ID_cantante -> ID_solista, ID_grupo
        
        Para solucionarlo pondríamos en una nueva tabla **CANTANTE** todos los atributos en común y luego las conectaríamos por sus IDs.
        
        CANTANTE(ID_cantante(PK), nombre, agencia, fecha_turno, género_musical, color_oficial, fecha_debut, nombre_fanclub, ID_maquillaje(FK), ID_vestuario(FK))
        
        SOLISTA (ID_solista(PK), num_bailarines, ID_cantante(FK))
                                        
        GRUPO (ID_grupo (PK), num_integrantes, ID_cantante(FK))
                  
        - **VESTUARIO** y **MAQUILLAJE**: No tendrían dependencias.
        
        - **FANCLUB**, **FAN** y **PROYECTO**: No tienen dependencias funcionales.
        
    - Tabla de **INVITACIONES**:
        
        - **INVITACION** y **CAMPO_TRABAJO**: Ya estarían en 2FN
        
        - **ACTOR/ACTRIZ** e **INFLUENCER**: Dependen de su ID de celebridad
        
        ID_celebridad -> ID_actor_actriz, ID_influencer
        
        Por lo que podríamos crear una tabla nueva para las celebridades.
        
        CELEBRIDAD (ID_celebridad, ID_actor_actriz, ID_influencer)
        
        ACTOR/ACTRIZ (ID_actor_actriz, nombre, agencia)
        
        INFLUENCER (ID_influencer, nombre, red_social)

    - Tabla de **CARTELES**:
    
        - **DISEÑADOR** y **CARTEL**: Ya estan en 2FN
        
    - Tabla de **AGENDA**:
    
        - **AGENDA** y **HORARIO**: Ya estarían en 2FN
        
- **Tercera Forma Normal (3FN)**

    - Tabla de **ORGANIZACION**: Puede haber una dependencia transitiva en presupuesto.
    
    presupuesto -> empresa
    
    Pero al ser una tabla que tiene solo un valor por columna sería deficiente dividirla en más de una tabla.
    
    - El resto de las tablas deberían estar tambien en 3FN.
    
- **FORMA FINAL**:

    - **Tabla de ORGANIZACION**:
        ORGANIZACIÓN (ID_organizacion(PK), nombre, jefe/jefa, empresa, presupuesto, patrocinador)
        
    - **Tablas de ARTISTAS**:
    
        ARTISTA (ID_artista(PK), ID_organización(FK), ID_cartel(FK), ID_agenda(FK), ID_speaker(PK)(FK), ID_presentador(PK)(FK), ID_cantante(PK)(FK))
        
        SPEAKER (ID_speaker(PK), nombre, agencia, fecha_turno)
        
        CHARLA (ID_speaker(PK)(FK), tema_charla(PK))
                                
        CANTANTE(ID_cantante(PK), nombre, agencia, fecha_turno, género_musical, color_oficial, fecha_debut, nombre_fanclub, ID_maquillaje(FK), ID_vestuario(FK))
                
        SOLISTA (ID_solista(PK), num_bailarines, ID_cantante(FK))
                                                
        GRUPO (ID_grupo (PK), num_integrantes, ID_cantante(FK))  
            
        PRESENTADOR (ID_presentador(PK), nombre, agencia, fecha_turno)
        
        MAQUILLADOR (ID_maquillador, nombre, contacto, red_social, material, especialidad)
                
        VESTUARIO (ID_vestuario, nombre, contacto, red_social, marca_ropa)
        
        FANCLUB (ID_fanclub(PK), nombre, página_web, ID_cantante(FK))
        
        FAN (ID_fan(PK), nombre, fecha_inscripción, estatus, ID_fanclub(FK))
        
        PROYECTO (ID_proyecto(PK), descripción, material, ID_FAN(FK))
        
    - **Tablas de INVITACIONES**:
    
        INVITACIÓN (ID_invitación(PK), código_QR, fecha, ID_celebridad(FK), ID_organizacion(FK))
        
        CELEBRIDAD (ID_celebridad(PK), ID_actor_actriz(FK), ID_influencer(FK))
        
        ACTOR/ACTRIZ (ID_actor_actriz(PK), nombre, agencia)
        
        INFLUENCER (ID_influencer(PK), nombre, red_social)
        
        CAMPO_TRABAJO (ID_celebridad(PK)(FK), campo(PK))
        
    - **Tablas de CARTELES**:
    
        DISEÑADOR (ID_diseñador(PK), nombre, red_social,ID_organizacion(FK))
        
        CARTEL (ID_cartel(PK), presupuesto, fecha, ID_diseñador(FK), ID_agenda(FK))
        
    - **Tablas de AGENDA**:
    
        AGENDA (ID_agenda(PK), fecha, previsión_día, ID_organizacion(FK))
        
        HORARIO (ID_horario(PK), fecha, hora, descripción_evento, ID_agenda(FK))
